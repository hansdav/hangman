...
const answer = WORDS_TO_GUESS[Math.floor(Math.random() * WORDS_TO_GUESS.length)].toString().split('');
...
for (let i = 0; i < answer.length; i++) {
  if (guess.length != 1 || guess === null || guess === '' || Number.isInteger(parseInt(guess.toString())) === true) {
...
if (incorrect === answer.length) {
  console.log(`Oops. That was not the right letter. You now have ${life} left.`)
...
while (rightGuess !== answer.length && life !== 0) {
  matchFunc();
}
